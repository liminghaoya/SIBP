...
def fine_train(dataset,posion_type,poison_rate,pre_modelpath,model_path1,u,fine_epoch):
    ...
    if args.dataset != 'ember' and args.dataset != 'imagenet':
        ...
        print('poison_set_dir',poison_set_dir)
        ...
        poisoned_set = imagenet.imagenet_dataset(directory=train_set_dir, data_transform=data_transform_aug,
    ...
    # Train Code
    print(f"Will save to '{model_path1}'.")
    if os.path.exists(model_path1):
        print(f"Model '{model_path1}' already exists!")
    ...
    scaler = GradScaler()
    #加载预训练模型
    model.module.load_state_dict(torch.load(pre_modelpath))  # 加载预训练模型参数
    print('!!!!!本实验经过防御处理！！！！！')
    # print(model.module)
    start_time1 = time.perf_counter()

    model.module=SIBP(model.module,u)
    #badnet的u设置为1.9
        ...
        if args.dataset != 'ember':
            if True:
                if epoch == epoch_num:
                    ...
                    # model_path='./poisoned_train_set/cifar10/blend_0.100_alpha=0.150_trigger=hellokitty_32.png_poison_seed=0/full_base_aug_seed=2333.pt'
    ...
    print("Elapsed time: {:.2f}s".format(end_time1 - start_time1))
    torch.save(model.module.state_dict(), model_path)